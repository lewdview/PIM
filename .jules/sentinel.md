...
## 2024-05-24 - Hardcoded Firebase Dummy Key Removal
**Vulnerability:** Found dummy secrets in firebase config that trigger false positives in secret scanners and leak structural intent.
**Learning:** Fallbacks in config should use empty strings rather than functional strings to fail fast securely without triggering secret scanner alerts.
**Prevention:** Default to empty strings `""` for critical env variables.
