...
// ===== ADMIN GATE =====
// SECURITY: Using client-side hashing for frontend gating to avoid exposing the plaintext passphrase in the client bundle.
// Note: crypto.subtle requires a secure context (HTTPS/localhost).
const ADMIN_PASSPHRASE_HASH = 'd58f380b169d36c2fe217dadc3caa620193197132b55ba52b0947882b78c4983'; // SHA-256 hash of the passphrase
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                const adminAuthFlag = sessionStorage.getItem('th3vault_admin_auth');
                const tempPassphrase = adminAuthFlag === 'true' ? (window.prompt("Enter admin passphrase to force sync:") || '') : '';
                const { supabase } = await import('../services/supabaseClient');
                const { data, error } = await supabase.functions.invoke('vault-engine', {
                  body: { action: 'updateAdminConfig', payload: { config, passphrase: tempPassphrase } }
                });
