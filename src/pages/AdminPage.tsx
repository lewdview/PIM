...
function AdminGate({ onAuthenticate }: { onAuthenticate: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hashHex === ADMIN_PASSPHRASE_HASH) {
      ...
      sessionStorage.setItem('th3vault_admin_pass', input);
      onAuthenticate();
    } else {
      setError(true);
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const adminPass = sessionStorage.getItem('th3vault_admin_pass') || prompt('Enter admin passphrase to authenticate push');
                const { data, error } = await supabase.functions.invoke('vault-engine', {
                  body: { action: 'updateAdminConfig', payload: { config, passphrase: adminPass } }
                });
