...
function AdminGate({ onAuthenticate }: { onAuthenticate: (passphrase: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Convert input to lowercase before hashing for consistency
...
export default function AdminPage() {
  ...
  const [ephemeralPassphrase, setEphemeralPassphrase] = useState<string>('');
            ...
            onClick={async () => {
              ...
              try {
                ...
                const { data, error } = await supabase.functions.invoke('vault-engine', {
                  body: { action: 'updateAdminConfig', payload: { config, passphrase: ephemeralPassphrase || window.prompt('Enter admin passphrase to push config:') } }
                });
