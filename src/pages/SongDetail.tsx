...
import { getSongById, getModifierForSong } from "@/game/api";
...
export default function SongDetail() {
  ...
  const handlePlay = () => {
    ...
    const isEquipped = equippedCardId === song.id;
    if (isEquipped && modifierType !== 'none') {
      sessionStorage.setItem(`active_modifier_type_${songId}`, modifierType);
    } else {
      sessionStorage.removeItem(`active_modifier_type_${songId}`);
    }
            ...
            <div className="relative flex-shrink-0 group">
              ...
              {song.coverArt ? (
                <img src={song.coverArt} alt={song.title}
                  ...
                  style={{ width: 88, height: 88, border: '1px solid rgba(57,255,20,0.2)', filter: unlocked ? 'grayscale(15%)' : 'grayscale(100%) brightness(0.4)' }} />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                {unlocked ? (
                  ...
                  <span className="font-mono text-[8px] px-1.5 py-[2px] border border-[#FF3800] text-[#FF3800] bg-[#FF3800]/10 font-black uppercase">
                    LOCKED
                  </span>
                )}
                {medal && medal !== '' && (
              ...
              <h1 className="font-mono font-black leading-tight uppercase tracking-tight text-[18px]"
                style={{ color: '#F2F0E8', opacity: unlocked ? 1 : 0.5 }}>
                {song.title}
              </h1>
          ...
          {unlocked && hasCard && modifierType !== 'none' && (
            <div className="border border-[#39FF14]/30 bg-black/40 p-3 flex items-center justify-between mb-2">
              <div className="flex-1 pr-2 text-left">
                <div className="font-mono text-[8px] text-[#39FF14] font-black uppercase tracking-wider">
                  // MODIFIER CARD OWNED
                </div>
                <div className="font-mono text-[10px] text-white/80 font-bold mt-0.5 uppercase">
                  {modifierType === 'vocal_isolation' ? 'VOCAL ISOLATION' :
                   modifierType === 'bass_realm' ? 'BASS REALM' :
                   modifierType === 'corrupted_signal' ? 'CORRUPTED SIGNAL' : modifierType}
                </div>
                <div className="font-mono text-[8px] text-white/40 mt-0.5 uppercase leading-normal">
                  {modifierType === 'vocal_isolation' ? 'BOOST VOCAL CHANNELS · DAMPEN BASS & TREBLE' :
                   modifierType === 'bass_realm' ? 'INTENSIFY BASS + WIDER HIGHWAY LANE-0 NOTES' :
                   modifierType === 'corrupted_signal' ? 'TEMPO DRIFT & CRT VISUAL GLITCH STIMULATION' : ''}
                </div>
              </div>
              <button
                onClick={() => {
                  audioManager.playSfx('tap_nav', 0.08);
                  setEquippedCardId(equippedCardId === song.id ? null : song.id);
                }}
                className={`font-mono text-[9px] font-black px-3 py-2 border uppercase transition-colors cursor-pointer ${
                  equippedCardId === song.id
                    ? 'border-[#39FF14] bg-[#39FF14]/20 text-[#39FF14]'
                    : 'border-[#39FF14]/30 bg-transparent text-white/50 hover:border-[#39FF14]/50'
                }`}
              >
                {equippedCardId === song.id ? 'ACTIVE' : 'EQUIP'}
              </button>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: 'radial-gradient(ellipse 80% 50% at 50% 20%, #0e1028 0%, #080808 60%)' }}>
      ...
      <div className="relative z-10 max-w-xl mx-auto px-4 py-6 space-y-5 slide-up">
          ...
          <div className="relative flex-shrink-0 group">
            {song.coverArt ? (
              <img src={song.coverArt} alt={song.title}
                ...
                style={{ width: 96, height: 96, boxShadow: `0 8px 28px rgba(0,0,0,0.5), 0 0 16px ${moodColor}15`, filter: unlocked ? 'none' : 'grayscale(100%) brightness(0.4)' }} />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              {unlocked ? (
                ...
                <span className="pill-badge border border-[#FF3800] text-[#FF3800] bg-[#FF3800]/10"
                  style={{ boxShadow: `0 0 8px rgba(255,56,0,0.2)` }}>
                  LOCKED
                </span>
              )}
              {medal && medal !== '' && (
            ...
            <h1 className="font-mono font-bold leading-tight mb-1"
              style={{ fontSize: 'clamp(15px, 4vw, 20px)', color: '#F2F0E8', opacity: unlocked ? 1 : 0.5 }}>
              {song.title}
            </h1>
        ...
        {/* Functional Card Modifier Toggle */}
        {unlocked && hasCard && modifierType !== 'none' && (
          <div className="glass-panel p-3.5 flex items-center justify-between mb-2" style={{ borderRadius: 12 }}>
            <div className="flex-1 pr-2 text-left">
              <div className="font-mono text-[8px] text-[#ffd700] font-black uppercase tracking-wider">
                // MODIFIER CARD OWNED
              </div>
              <div className="font-mono text-[10px] text-white/80 font-bold mt-0.5 uppercase">
                {modifierType === 'vocal_isolation' ? 'VOCAL ISOLATION' :
            ...
            <button
              ...
              className={`font-mono text-[9px] font-black px-3 py-2 border rounded-lg uppercase transition-colors cursor-pointer ${
                equippedCardId === song.id
        ...
        <div className="pb-4 flex flex-col gap-2">
          ...
          <button onClick={handlePlay}
            disabled={!unlocked}
            className={`w-full py-5 text-sm tracking-[0.4em] uppercase rounded-xl transition-all ${
              unlocked 
                ? "neon-btn" 
