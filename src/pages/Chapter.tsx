      ...
      <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col lg:flex-row gap-8 relative z-10">
        ...
        <div className="w-full lg:w-[40%] flex flex-col gap-6 lg:sticky lg:top-20 lg:h-[calc(100vh-120px)] lg:overflow-y-auto pr-1">
          ...
          <div className={isAvant ? 'border border-[#39FF14]/25 bg-black/60 p-4' : 'glass-panel p-4 rounded-xl'}
            ...
            {/* Milestones claim slots */}
            <div className="space-y-3">
              {/* Milestone 1 (5 Clears) */}
              <div className="flex items-center justify-between p-2.5 rounded border"
                style={{
                  background: 'rgba(255,255,255,0.01)',
                  borderColor: clearsCount >= 5 ? (claimedM1 ? 'rgba(57,255,20,0.1)' : '#E5B80030') : 'rgba(255,255,255,0.04)'
                }}>
                <div className="flex items-center gap-2">
                  <span className="text-xl">🎧</span>
                  <div className="text-left">
                    <div className="font-mono font-bold text-[10px] text-[#F2F0E8]">TASTE PACK (1 CARD)</div>
                    <div className="font-mono text-[8px] text-white/40">MILESTONE 1: 5 SEC CLEAR REWARD</div>
                  </div>
                </div>
                {claimedM1 ? (
                  <span className="font-mono font-bold text-[9px] text-[#39FF14] bg-[#39FF14]/10 px-2 py-1 rounded">CLAIMED ✓</span>
                ) : clearsCount >= 5 ? (
                  <button onClick={() => claimReward(1)}
                    className="font-mono font-black text-[9px] bg-gradient-to-r from-[#E5B800] to-[#FFD700] hover:scale-105 transition-transform text-black px-3 py-1 rounded shadow cursor-pointer">
                    CLAIM GIFT
                  </button>
                ) : (
                  <span className="font-mono text-[9px] text-white/30 border border-white/5 px-2 py-1">5 CLEARS REQ</span>
                )}
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded border"
                style={{
                  background: 'rgba(255,255,255,0.01)',
                  borderColor: clearsCount >= 15 ? (claimedM2 ? 'rgba(57,255,20,0.1)' : '#E5B80030') : 'rgba(255,255,255,0.04)'
                }}>
                <div className="flex items-center gap-2">
                  <span className="text-xl">{meta.mood === 'light' ? '☀️' : '🌙'}</span>
                  <div className="text-left">
                    <div className="font-mono font-bold text-[10px] text-[#F2F0E8]">
                      {meta.mood.toUpperCase()} PACK (1 CARD)
                    </div>
                    <div className="font-mono text-[8px] text-white/40">MILESTONE 2: 15 SEC CLEAR REWARD</div>
                  </div>
                </div>
                {claimedM2 ? (
                  <span className="font-mono font-bold text-[9px] text-[#39FF14] bg-[#39FF14]/10 px-2 py-1 rounded">CLAIMED ✓</span>
                ) : clearsCount >= 15 ? (
                  <button onClick={() => claimReward(2)}
                    className="font-mono font-black text-[9px] bg-gradient-to-r from-[#E5B800] to-[#FFD700] hover:scale-105 transition-transform text-black px-3 py-1 rounded shadow cursor-pointer">
                    CLAIM GIFT
                  </button>
                ) : (
                  <span className="font-mono text-[9px] text-white/30 border border-white/5 px-2 py-1">15 CLEARS REQ</span>
                )}
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded border"
                style={{
                  background: 'rgba(255,255,255,0.01)',
                  borderColor: clearsCount >= regularSongs.length ? (claimedM3 ? 'rgba(57,255,20,0.1)' : '#E5B80030') : 'rgba(255,255,255,0.04)'
                }}>
                <div className="flex items-center gap-2">
                  <span className="text-xl">{meta.mood === 'light' ? '🌙' : '☀️'}</span>
                  <div className="text-left">
                    <div className="font-mono font-bold text-[10px] text-[#F2F0E8]">
                      {meta.mood === 'light' ? 'DARK' : 'LIGHT'} PACK (1 CARD)
                    </div>
                    <div className="font-mono text-[8px] text-white/40">MILESTONE 3: ALL REGULAR CLEARED</div>
                  </div>
                </div>
                {claimedM3 ? (
                  <span className="font-mono font-bold text-[9px] text-[#39FF14] bg-[#39FF14]/10 px-2 py-1 rounded">CLAIMED ✓</span>
                ) : clearsCount >= regularSongs.length ? (
                  <button onClick={() => claimReward(3)}
                    className="font-mono font-black text-[9px] bg-gradient-to-r from-[#E5B800] to-[#FFD700] hover:scale-105 transition-transform text-black px-3 py-1 rounded shadow cursor-pointer">
                    CLAIM GIFT
                  </button>
                ) : (
                  <span className="font-mono text-[9px] text-white/30 border border-white/5 px-2 py-1">ALL CLEARS REQ</span>
                )}
              </div>
            </div>
          </div>
          ...
          {selectedSong ? (
            <div className={isAvant ? 'border border-[#39FF14]/30 bg-black/55 p-5 text-left space-y-4' : 'glass-panel p-5 rounded-xl text-left space-y-4'}
              ...
              <div className="flex gap-4 items-start">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap mb-1">
                    ...
                    {ownsCard && (
                      <span className="font-mono text-[8px] px-1.5 py-px border border-yellow-500/40 text-yellow-500 bg-yellow-500/10 rounded uppercase font-black">
                        OWNED 💳
                      </span>
                    )}
                  </div>
              ...
              {isUnlocked(songs.indexOf(selectedSong)) && ownsCard && modifierType !== 'none' && (
                <div className="border border-[#39FF14]/25 bg-black/40 p-2.5 flex items-center justify-between rounded">
                  <div className="text-left pr-2">
                    <div className="font-mono text-[8px] text-[#39FF14] font-black uppercase">
                  ...
                  <button
                    ...
                    className={`font-mono text-[9px] font-black px-2.5 py-1.5 border rounded uppercase transition-colors cursor-pointer ${
